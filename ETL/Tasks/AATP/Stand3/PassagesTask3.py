...
class stand3PeriodsWritterBd(Construct):
    def __init__(self):
        super(stand3PeriodsWritterBd, self).__init__()
        ...
        self.pg_query_read_calcperiods = """
                                ...
                                JOIN aatp.stand3_passageway m ON m.ref_nomenclature = t.ref_nomenclature AND t.dt_start = m.dt_start
                                ORDER BY t.dt_start
                                """
...
process = stand3PeriodsWritterBd()
while True:
    try:
        process.Main()
    except:
