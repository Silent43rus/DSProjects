...
class cuttingPeriogsWritter(Construct):
    def __init__(self):
        super(cuttingPeriogsWritter, self).__init__()
        ...
        self.pg_query_read_calcperiods = """
                                ...
                                JOIN aatp.cutting_cuts m ON m.ref_nomenclature = t.ref_nomenclature AND t.dt_start = m.dt_start
                                ORDER BY t.dt_start
                                """
    ...
    def Main(self):
        try:
            ...
            res = self.logWritter(self.script_name, self.file_directory, df)
