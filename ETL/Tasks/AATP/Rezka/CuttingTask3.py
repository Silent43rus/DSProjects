...
class stand3UpdateAccept(Construct):
    def __init__(self):
        ...
        self.pg_query_read_periods = """
                                ...
                                """
        self.pg_query_read_passageway = """ SELECT dt_start, dt_stop, cut_number as num_proh, amperage_mean, amperage_max, power_mean,
                                                power_max, vibration_mean, vibration_max, interformation_pause,
                                                time_cycle, time_rez, disc_speed, disc_diameter
        ...
        try:
            ...
            for par in range(len(listpar)):
                for i in range(1, maxproh + 1):
                    temparr = newdf[(newdf['cycle'] == i) & (newdf[f'{listpar[par]}'] > 0)][f'{listpar[par]}']
